...
---
## 최신 변경 사항
### v1.2 보스 부위 HP HUD 씬 연결 완료
- P1 `F-12 보스 HP 바`의 실제 Game 씬 HUD에 HEAD, ARM L/R, LEG L/R, CHEST, CORE 7개 부위별 HP 슬라이더와 상태 텍스트를 연결했다.
- `SceneBuilder`가 배치 모드에서도 팝업 없이 Game/Result 씬을 재생성하도록 보정해 빌드 자동화 흐름을 안정화했다.
- `Game.unity`에 `PartHpSlider_*`, `PartHpText_*`, `partHpSliders`, `partHpTexts` serialized 참조가 저장되는지 검증했다.
- EditMode 씬 검증 테스트를 추가해 Boss HUD가 7개 부위 HP 컨트롤을 모두 참조하는지 확인한다.
